...
# ── 핫픽스: 전각 괄호 정규화 (실제 발굴함 실패 8건 근본 해결) ──────────────────
# 데이터 시트 차종명에 전각 괄호 U+FF08（ / U+FF09）혼입(짝짝이 포함) → 괄호·슬래시가
# 키워드에 잔존 → 검색광고 키워드도구 400(code 11001). 원문을 전각 그대로 픽스처로 고정.
_FAILING_8 = [
    ("코란도 (뷰티풀/리스펙）", "코란도"),      # ★반각으로 열고 전각으로 닫는 짝짝이 + 슬래시
    ("2시리즈（액티브 투어러）", "2시리즈"),     # 트림 3건 → 2시리즈로 수렴
    ("2시리즈（그란 쿠페）", "2시리즈"),
    ("2시리즈（쿠페）", "2시리즈"),
    ("3시리즈（세단）", "3시리즈"),
    ("5시리즈（투어링）", "5시리즈"),
...
def test_normalize_fullwidth_paren_failing_cases():
    """전각/짝짝이 괄호 8건 → 기대 정규명, 그리고 build_keyword(와이퍼)까지 일관 적용."""
    for raw, expected in _FAILING_8:
        assert normalize_car_keyword(raw) == expected, raw
        assert build_keyword(raw, "와이퍼") == f"{expected}와이퍼", raw


def test_normalize_fullwidth_no_forbidden_chars():
    """정규화 결과에 키워드도구 비허용 문자([^가-힣A-Za-z0-9])가 하나도 없어야 한다."""
    for raw, _expected in _FAILING_8:
        out = normalize_car_keyword(raw)
        assert re.search(r"[^가-힣A-Za-z0-9]", out) is None, raw


def test_normalize_2series_trims_converge():
    """2시리즈 트림 3건이 동일 키워드(2시리즈와이퍼)로 수렴한다."""
    kws = {build_keyword(raw, "와이퍼") for raw, exp in _FAILING_8 if exp == "2시리즈"}
    assert kws == {"2시리즈와이퍼"}


def test_normalize_halfwidth_and_space_unchanged():
    """기존 정상 차종명(반각 괄호·공백) 출력 불변 — 회귀 방지."""
    assert normalize_car_keyword("콜레오스(QM6)") == "콜레오스"
    assert normalize_car_keyword("QM6 (콰트로)") == "QM6"
    assert normalize_car_keyword("그랑 콜레오스") == "그랑콜레오스"
    assert normalize_car_keyword("셀토스") == "셀토스"
